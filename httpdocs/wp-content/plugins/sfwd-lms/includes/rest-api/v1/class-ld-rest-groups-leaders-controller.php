...
if ( ( !class_exists( 'LD_REST_Groups_Leaders_Controller_V1' ) ) && ( class_exists( 'LD_REST_Users_Controller_V1' ) ) ) {
	class LD_REST_Groups_Leaders_Controller_V1 extends LD_REST_Users_Controller_V1 {
		...
		function update_groups_leaders_permissions_check( $request ) {
			if ( learndash_is_admin_user( ) ) {
				return true;
			}
		}

		function delete_groups_leaders_permissions_check( $request ) {
			if ( learndash_is_admin_user( ) ) {
				return true;
			}
		}

		function update_groups_leaders( $request ) {
			$group_id = $request['id'];
			if ( empty( $group_id ) ) {
		...
		function delete_groups_leaders( $request ) {
			$group_id = $request['id'];
			if ( empty( $group_id ) ) {
				return new WP_Error( 'rest_post_invalid_id', esc_html__( 'Invalid group ID.', 'learndash' ) .' '. __CLASS__, array( 'status' => 404 ) );
			}
