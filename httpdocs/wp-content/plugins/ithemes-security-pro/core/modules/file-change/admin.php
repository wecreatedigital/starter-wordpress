...
final class ITSEC_File_Change_Admin {
	...
	public function dismiss_file_change_warning_ajax() {
		if ( ! wp_verify_nonce( $_REQUEST['nonce'], 'itsec-file-change-dismiss-warning' ) ) {
			wp_send_json_error( array(
				'message' => __( 'Request expired. Please refresh and try again.', 'it-l10n-ithemes-security-pro' ),
			) );
		}
		...
		if ( ! $status || empty( $status['saved'] ) ) {
			wp_send_json_error( array(
				'message' => __( 'Failed to dismiss warning.', 'it-l10n-ithemes-security-pro' ),
			) );
		}

		wp_send_json_success( array(
			'message' => __( 'Warning dismissed.', 'it-l10n-ithemes-security-pro' ),
		) );
	}
	...
	public function show_file_change_warning() {
		...
		$message  = sprintf(
			esc_html__( 'iThemes Security noticed file changes in your WordPress site. Please %1$s review the logs %2$s to make sure your system has not been compromised.', 'it-l10n-ithemes-security-pro' ),
